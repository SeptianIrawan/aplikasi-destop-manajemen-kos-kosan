...
//        this.setExtendedState(Dashboard.MAXIMIZED_BOTH);
    ...
    private void initComponents() {
        ...
        getContentPane().add(tpassword, new org.netbeans.lib.awtextra.AbsoluteConstraints(310, 170, 240, 30));

        clevel.setModel(new javax.swing.DefaultComboBoxModel<>(new String[] { "PILIH", "ADMIN", "KARYAWAN" }));
        getContentPane().add(clevel, new org.netbeans.lib.awtextra.AbsoluteConstraints(680, 120, 240, 30));
        ...
        jPanel3Layout.setHorizontalGroup(
            ...
            .addGroup(jPanel3Layout.createSequentialGroup()
                ...
                .addGroup(jPanel3Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    .addGroup(jPanel3Layout.createSequentialGroup()
                        .addComponent(tusername, javax.swing.GroupLayout.PREFERRED_SIZE, 240, javax.swing.GroupLayout.PREFERRED_SIZE)
                        .addGap(24, 403, Short.MAX_VALUE))
                    .addGroup(jPanel3Layout.createSequentialGroup()
                        .addGap(285, 285, 285)
    ...
    try {
    // Validasi data input sebelum menyimpan
    if (tusername.getText().trim().isEmpty() || 
        tpassword.getText().trim().isEmpty() || 
        clevel.getSelectedItem() == null) {
        JOptionPane.showMessageDialog(null, "Harap lengkapi semua data sebelum menyimpan.");
        return; // Hentikan eksekusi jika ada input yang kosong
    }

    // Query SQL untuk menyimpan data ke tabel `akun`
    String sql = "INSERT INTO akun (username, password, level, foto) VALUES (?, ?, ?, ?)";

    // Menggunakan PreparedStatement untuk menghindari SQL Injection
    PreparedStatement ps = con.prepareStatement(sql);

    // Mengisi parameter pada query SQL
    ps.setString(1, tusername.getText().trim());  // Username
    ps.setString(2, tpassword.getText().trim());  // Password (sebaiknya dienkripsi)
    ps.setString(3, clevel.getSelectedItem().toString().trim()); // Level
    ps.setString(4, ttampil.getText().trim().isEmpty() ? null : ttampil.getText().trim()); // Path atau nama file foto (boleh null)
...
if (row != -1) {
    String idPenyewa = tabelPenyewa.getValueAt(row, 1).toString(); // Ambil id_penyewa dari tabel

    // Pastikan data yang dibutuhkan sudah ada
    String username = tusername.getText();
    String password = tpassword.getText();
    String level = clevel.getSelectedItem().toString();
    String foto = ttampil.getText();

    if (username.isEmpty() || password.isEmpty() || level.isEmpty() || foto.isEmpty()) {
        JOptionPane.showMessageDialog(rootPane, "Semua field harus diisi");
    } else {
        try {
            // Periksa apakah data yang akan diupdate sudah sama dengan data yang ada
            ...
            if (rs.next()) {
                ...
                String currentFoto = rs.getString("foto");

                // Jika data sudah sama, jangan lakukan update
                if (username.equals(currentUsername) && password.equals(currentPassword) && 
                    level.equals(currentLevel) && foto.equals(currentFoto)) {
                    JOptionPane.showMessageDialog(rootPane, "Tidak ada perubahan data");
                } else {
                    // Jika data berbeda, lanjutkan dengan update
                    String sql = "UPDATE akun SET password = ?, level = ?, foto = ? WHERE username = ?";
                    PreparedStatement ps = con.prepareStatement(sql);
                    ...
                    if (rowsUpdated > 0) {
                        ...
                        JOptionPane.showMessageDialog(rootPane, "Tidak ada data yang diupdate");
                    }
                }
            } else {
...
if (row != -1) {
    String username = tabelPenyewa.getValueAt(row, 0).toString(); // Ambil username dari tabel
    System.out.println("Username yang akan dihapus: " + username);  // Debugging: Menampilkan username yang diambil
    ...
    if (confirm == JOptionPane.YES_OPTION) {
        try {
            ...
            if (rowsAffected > 0) {
                ...
                JOptionPane.showMessageDialog(rootPane, "Data tidak ditemukan atau gagal dihapus");
                System.out.println("Query SQL gagal dieksekusi, tidak ada baris yang terhapus");  // Debugging
            }
            ...
            System.out.println("Error: " + e.getMessage());  // Debugging: Menampilkan error yang terjadi
        }
    }
} else {
...
if (row != -1) {
    ...
    String idPenyewa = tabelPenyewa.getValueAt(row, 0).toString(); // Kolom username (0)
    String password = tabelPenyewa.getValueAt(row, 1).toString();  // Kolom password (1)
    String level = tabelPenyewa.getValueAt(row, 2).toString();     // Kolom level (2)
    String foto = tabelPenyewa.getValueAt(row, 3).toString();      // Kolom foto (3)

    // Mengisi JTextField dengan data dari tabel
    tusername.setText(idPenyewa);  // Mengisi data username
    tpassword.setText(password);  // Mengisi data password
    clevel.setSelectedItem(level);      // Mengisi data level

    // Menampilkan file path di JTextField ttampil
    if (foto != null && !foto.isEmpty()) {
        ttampil.setText(foto); // Menampilkan path gambar atau nama file ke dalam JTextField ttampil
    }
    ...
    if (foto != null && !foto.isEmpty()) {
        ...
        if (fotoFile.exists()) {
            // Mengatur gambar ke ukuran label
            ...
            lblfoto.setIcon(new ImageIcon(scaledImage)); // Menampilkan gambar pada JLabel
        } else {
            lblfoto.setIcon(null); // Jika file tidak ditemukan, kosongkan gambar
            JOptionPane.showMessageDialog(null, "File gambar tidak ditemukan: " + foto);
        }
    } else {
        lblfoto.setIcon(null); // Jika kolom foto kosong, kosongkan gambar
    }
} else {
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
       String username = tpencarian.getText().trim(); // Mengambil input dari JTextField
        ...
        try {
            ...
            if (res.next()) {
                ...
                model.addRow(new Object[]{
                 ...
                 tabelPenyewa.setModel(model); // Menampilkan hasil pencarian di JTable
                 JOptionPane.showMessageDialog(rootPane, "ini data akun dengan Username " +  tpencarian.getText());
            } else {
                JOptionPane.showMessageDialog(this, "Akun dengan Username " + username + " tidak ditemukan.", "Informasi", JOptionPane.INFORMATION_MESSAGE);
...
if (result == JFileChooser.APPROVE_OPTION) {
    ...
    tusername.setText("");
    tpassword.setText("");
    clevel.setSelectedIndex(0); // Reset combo box ke pilihan pertamA
    ttampil.setText("");
    ...
    private void tampildata() {
       DefaultTableModel model = new DefaultTableModel();
        ...
        try {
            ...
            while (res.next()) {
                // Menambahkan baris tanpa kolom password
                model.addRow(new Object[]{
                    res.getString("username"),
                     res.getString("password"),
                    res.getString("level"),
